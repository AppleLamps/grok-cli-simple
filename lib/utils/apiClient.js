...
class APIClient {
  constructor(apiKey, model, options = {}) {
    ...
    this.timeout = options.timeout || parseInt(process.env.API_TIMEOUT || '30000', 10);
   ...
   * @returns {Promise<Object>} Object with message and usage data: {message: {...}, usage: {...}}
   */
  ...
  async _makeRequestWithRetry(messages, options, attemptsRemaining) {
    ...
    try {
      ...
      // Return message object with usage data
      return {
      ...
      if (isRetryable && attemptsRemaining > 0) {
        spinner.text = `Retrying... (${4 - attemptsRemaining} attempts)`;
